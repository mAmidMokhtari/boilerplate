...
function request(path: string, init: { method?: string; cookies?: Record<string, string>; headers?: Record<string, string> } = {}) {
  const req = new NextRequest(`https://app.test/api/backend/${path}`, { method: init.method ?? "GET", headers: init.headers });
...
describe("createBackendProxy", () => {
  ...
  it("rejects paths outside the allow-list", async () => {
    const proxy = createBackendProxy({ backend: makeBackend(), cookies: { names, secure: false }, allowedPrefixes: ["admin/v1/"] });
  ...
  it("forwards with the bearer from the cookie", async () => {
    fetchMock.mockResolvedValueOnce(new Response(JSON.stringify({ data: 1 }), { status: 200, headers: { "content-type": "application/json" } }));
    const proxy = createBackendProxy({ backend: makeBackend(), cookies: { names, secure: false }, allowedPrefixes: ["admin/v1/"] });
    const res = await proxy(request("admin/v1/posts?page=2", { cookies: { t_access: "abc" } }), ctx("admin/v1/posts"));
  ...
  it("refreshes once on 401 and rotates cookies", async () => {
    ...
    const backend = makeBackend({ refresh: vi.fn(async () => ({ access_token: "new", refresh_token: "new-r" })) });
    const proxy = createBackendProxy({ backend, cookies: { names, secure: false }, allowedPrefixes: ["admin/v1/"] });

    const res = await proxy(request("admin/v1/me", { cookies: { t_access: "old", t_refresh: "r" } }), ctx("admin/v1/me"));
    expect(res.status).toBe(200);
  ...
  it("blocks cross-origin mutations", async () => {
    const proxy = createBackendProxy({ backend: makeBackend(), cookies: { names, secure: false }, allowedPrefixes: ["admin/v1/"] });
    const res = await proxy(request("admin/v1/posts", { method: "POST", headers: { origin: "https://evil.test", host: "app.test" } }), ctx("admin/v1/posts"));
